...
6. Перейдите по адресу `http://localhost:8000`.
...
Для Docker используется хост базы `db`, а для локального запуска в `.env.example`
указан `localhost`.
...
Docker-образ использует Python 3.13. Для локальных проверок создайте виртуальное
окружение на Python 3.13 перед установкой `requirements.txt`.
